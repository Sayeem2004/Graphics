...
pub fn number_spiral(img : &mut Image, scale1: f32, scale2: f32) {
    ...
    for i in 0..img.get_height() {
        for q in 0..img.get_width() {
            ...
            let r : u8 = 0;
            let g : u8 = (number_spiral_helper(((i as f32) * scale1) as i32, ((q as f32) * scale1) as i32) % 256) as u8;
            let b : u8 = (number_spiral_helper(((q as f32) * scale2) as i32, ((i as f32) * scale2) as i32) % 256) as u8;
...
pub fn number_grid(img : &mut Image, scale1 : f32, scale2 : f32) {
    ...
    for i in 0..img.get_height() {
        for q in 0..img.get_width() {
            // Initializing rgb colors
            let r : u8 = 0;
            let g : u8 = (number_grid_helper(((i as f32) * scale1) as i32, ((q as f32) * scale1) as i32) % 256) as u8;
            let b : u8 = (number_grid_helper(((q as f32) * scale2) as i32, ((i as f32) * scale2) as i32) % 256) as u8;
...
pub fn barnsley(img : &mut Image, pix : Pixel, itr : i32, scale : f32) {
    ...
    for i in 1..itr {
        ...
        if (prob == 1) {
            ...
            yval[i as usize] = ((-0.04 * (xval[(i-1) as usize])) + (0.85 * (yval[(i-1) as usize])) + (img.get_width() as f32)/scale);
        } else if (prob <= 93) {
            // The large left leaflet
            xval[i as usize] = ((0.2 * (xval[(i-1) as usize])) - (0.26 * (yval[(i-1) as usize])));
            yval[i as usize] = ((0.23 * (xval[(i-1) as usize])) + (0.22 * (yval[(i-1) as usize])) + (img.get_width() as f32)/scale);
        } else {
            // The large right leaflet
            xval[i as usize] = ((-0.15 * (xval[(i-1) as usize])) + (0.28 * (yval[(i-1) as usize])));
            yval[i as usize] = ((0.26 * (xval[(i-1) as usize])) + (0.24 * (yval[(i-1) as usize])) + (img.get_width() as f32)/(scale*4.0));
        }
    }
    ...
    for i in 0..itr {
        ...
        img.update_pixel_xy((xval[i as usize] + ((width as f32/(scale/3.3)))) as i32, yval[i as usize] as i32, pix)
    }
}
