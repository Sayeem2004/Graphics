// Imports
use crate::algorithm::{extra, shape};
...
/// Function that performs the 'mesh' command
pub fn mesh(
    op: &Operation,
    symbols: &HashMap<String, Vec<Symbol>>,
    cord: &Matrix,
    ...
    let path: String = (*op.args.as_ref().unwrap()[0].as_string().unwrap()).clone();
    let mut poly = extra::parse_mesh(&path);

    // Adding object to image
    poly.left_transform(cs);
    poly.draw_triangles_xy(img, ambient, &pnts, constant::ZVIEW, constants);
}
