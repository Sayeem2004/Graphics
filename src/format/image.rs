...
impl Image {
    ...
    pub fn update_pixel_rc(&mut self, row : i32, col : i32, r : u8, g : u8, b : u8) {
        // Within size range
    ...
    // Updating a certain pixel
    pub fn update_pixel_xy(&mut self, x : i32, y : i32, r : u8, g : u8, b : u8) {
        // Within size range
    ...
    pub fn update_pixel_rc2(&mut self, row : i32, col : i32, pix : Pixel) {
        // Within size range
        if (row >= 0 && row < self.height && col >= 0 && col < self.width) {
            // Within color range
    ...
    // Updating a certain pixel with another pixel
    pub fn update_pixel_xy2(&mut self, x : i32, y : i32, pix : Pixel) {
        // Within size range
        if (y >= 0 && y < self.height && x >= 0 && x < self.width) {
            // Within color range
            if (pix.get_red() <= self.max_color && pix.get_green() <= self.max_color && pix.get_blue() <= self.max_color) {
